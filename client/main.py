...
if __name__ == "__main__":
    ...
    with connect(connectURL, subprotocols=["fido.cable"],
        ...
        try:
            ...
            raw = websocket.recv(timeout=10)
            logging.info(f"RECV Post-handshake raw ({len(raw)} bytes): {raw.hex()}")
            post_hs = loads(unpad_message(receive_cipher.decrypt_with_ad(b"", raw)))
            logging.info(f"Post-handshake decrypted: {post_hs}")
            # array of strings representing supported features:
            features = post_hs.get(3, ['ctap'])
            logging.info(f"Post-handshake cached features: {features}")
            assert 'dc' in features 
